...
const Character = (props) => {
  ...
  let description = null,characters=null,creators=null,comics=null,stories=null,events=null,dates='';
...
creators = characterData.creators.items.map((d,index) => <li key={`creator-${d.name}${index}`}>{d.name}</li>);
  }
  else{
...
events = characterData.events.items.map((d,index) => <li key={`events-${d.name}${index}`}>{d.name}</li>);
  ...
  if(characterData && characterData.startYear && characterData.endYear){
    dates=`${characterData.startYear}-${characterData.endYear}`;
  }
  if(characterData && characterData.rating){
    dates+=`  (${characterData.rating})`
  }
      ...
      <Fragment>
        <div className={classes.container}>
          ...
          >
            ...
            <StyledTab value='1'  label="Comics" className={classes.tab}/>
            <StyledTab value='2'  label="Creators" className={classes.tab}/>
            <StyledTab value='3' label="Events" className={classes.tab}/>
            <StyledTab value='4' label="Stories" className={classes.tab}/>
          </StyledTabs>
